...
public class UserController {
    ...
    public UserController(UserRepository userRepository, RoleRepository roleRepository, BCryptPasswordEncoder bCryptPasswordEncoder, PublicationRepository publicationRepository, StoreFileService storeFileService, EventRepository eventRepository) {
    ...
    public ResponseEntity<Void> newUser(@RequestBody CreateUserDto dto) {
        ...
        var userByCpf = userRepository.findByCpf(dto.cpf());

        if (userFromDb.isPresent() || userByCpf.isPresent()) {
            throw new ResponseStatusException(HttpStatus.UNPROCESSABLE_ENTITY, "Email ou CPF já cadastrado");
        }
    ...
    public ResponseEntity<Void> deleteUser(@PathVariable UUID id) {
        ...
        var events = eventRepository.findByUser(user);
    ...
    public ResponseEntity<Void> updateUser(@RequestBody UpdateUserDto dto) {
        ...
        if (dto.telephone() != null && !dto.telephone().isEmpty()) {
            user.setTelephone(dto.telephone());
        }
    ...
    public ResponseEntity<Void> deleteUser() {
        ...
        // Deletar todos os eventos relacionados ao usuário
        var events = eventRepository.findByUser(user);
        for (Event event : events) {
            // Delete images from S3
            if (event.getImageUrl() != null && !event.getImageUrl().isEmpty()) {
    ...
    @GetMapping("/profile")
    public ResponseEntity<UserDataDto> getUserProfile() {
        var authentication = SecurityContextHolder.getContext().getAuthentication();
        var userId = UUID.fromString(authentication.getName());
